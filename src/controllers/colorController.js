...
const getAllColor = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json(new ApiResponse(colors, "All colors get successfully."));
});
const createColor = catchAsync(async (req, res, next) => {
  const { name, colorCode } = req.body;
  const color = await colorService.createColor({ name, colorCode });

  res.status(200).json(new ApiResponse(color, "Color created successfully."));
});
const updateColor = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json(new ApiResponse(color, "Color successfully updated."));
});
