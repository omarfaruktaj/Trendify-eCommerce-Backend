...
const getAllReviews = catchAsync(async (req, res, _next) => {
	...
	res
		.status(200)
		.json(new ApiResponse({ reviews }, 'Reviews successfully get.'));
});
...
const createReview = catchAsync(async (req, res, next) => {
	...
	res
		.status(200)
		.json(
			new ApiResponse({ review: CreatedReview }, 'Successfully review create'),
		);
});
...
const updateReview = catchAsync(async (req, res, next) => {
	...
	res
		.status(200)
		.json(
			new ApiResponse({ rating: reviewDoc }, 'Review successfully updated.'),
		);
});
...
const deleteReview = catchAsync(async (req, res, next) => {
	...
	res
		.status(200)
		.json(new ApiResponse({ review }, 'Review successfully deleted.'));
});
